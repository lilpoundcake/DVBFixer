"""Apply DVBFixer boundary refinement to a materialized Boltz-2 candidate."""
...
def _identity(line: str) -> AtomIdentity:
    return AtomIdentity(
...
def run(
    ...
    *,
    ...
    if len(request.gaps) != 1 or request.candidate_count != 1:
        raise ValueError("Boltz refinement smoke requires one gap and candidate")
    if restart_count < 0:
    ...
    output_path.write_text(
        _rewrite_generated_coordinates(candidate_text, refinement.coordinates_angstrom),
        encoding="ascii",
    )

    candidate = RunnerCandidate(
        candidate_id=f"boltz-2-refined-seed-{request.seeds[0]}",
    ...
    runner_result = RunnerResult(
        ...
        backend_provenance=BackendProvenance(
            backend="boltz-2-hook-spike+boundary-refinement",
    ...
    quality = candidate_quality(
        ...
        closure_passed="junction-peptide-connectivity"
