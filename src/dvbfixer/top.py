...
# Writers
# ---------------------------------------------------------------------------
def write_itp(chain_top, path, bonded_types):
    """Write a chain topology to an ITP file."""
    bt = bonded_types
...
def write_top(chain_tops, path, ff_name, water_model='tip3p', system_name='Protein',
              has_interchain_ss=False):
    """Write master .top file."""
    with open(path, 'w') as f:
        f.write("; Generated by dvbfixer top\n")
        f.write("; Force field: {}\n\n".format(ff_name))

        f.write(f'#include "{ff_name}/forcefield.itp"\n\n')

        # Include chain ITPs
        for ct in chain_tops:
            itp_name = f"{ct.name}.itp"
        ...
        f.write(f'; Include water topology\n')
        f.write(f'#include "{ff_name}/{water_itp}"\n\n')

        f.write(f'; Include ion topology\n')
        f.write(f'#include "{ff_name}/ions.itp"\n\n')
...
def main(argv=None):
    ...
    write_top(chain_tops, top_path, ff_name, args.water, system_name,
