...
- **Glycoprotein minimization in `minimize`**: Default minimizes the WHOLE system (protein + glycans + ligands) with the resolved `--ff` (AMBER14 + GLYCAM_06j-1 for GLYCAM-named glycoproteins, CHARMM36 for CHARMM inputs). Strip-and-splice mode (`--strip-heterogens`) is an opt-in protein-only flow with HETATM coords spliced back from the input. When the full-system path can't parametrize the residue set (e.g. bare PDB-named sugars without a template in either GLYCAM or CHARMM), the tool auto-falls back to strip-and-splice and `_rigid_track_glycan_trees` does Kabsch tracking + canonical trans-amide C1/HD21 placement to preserve the glycan geometry. For arbitrary unknown ligands, add `--parametrize-ligands` to build real GAFF2 + AM1-BCC templates on the fly via antechamber.
...
- **LYN hydrogen naming (ff14SB vs OpenMM `hydrogens.xml`)**: NOT a bug, but a common source of confusion. The AMBER14 ff14SB LYN residue template (`openmm/app/data/amber14/protein.ff14SB.xml`) defines exactly two NZ hydrogens: `HZ2` and `HZ3` (HZ1 absent). OpenMM's shared `hydrogens.xml` — used by `Modeller.addHydrogens` — gates HZ3 under `variant="LYS"` rather than `variant="LYN"`, so calling addHydrogens with the LYN variant produces `HZ1 + HZ2` instead. `dvbfixer.ffutils.variants.fix_lyn_hz_naming` renames `HZ1 → HZ3` after every Modeller.addHydrogens call so the output matches ff14SB. If you're reading raw `hydrogens.xml` and it looks like the FF wants HZ1+HZ2, you're reading the wrong file — the residue template in `protein.ff14SB.xml` is authoritative.
