...
| Diffusion protocol and RFdiffusion benchmark | partial | `model/diffusion/contract.py`, `masks.py`, `scope.py`, `geometry.py`, `boundary_refinement.py`, `runner.py`, `validate.py`, `pipeline.py`, `preflight.py`, `provenance.py`, `benchmark.py`, `sampler.py`, `rfdiffusion_v1.py` | Versioned JSON contract, deterministic masks/geometry, fail-closed initial-slice scope admission, contained subprocess protocol, independent scientific gates/ranking, candidate-matched DatRecord generation, separate sanitized provenance, atomic no-replace bundle publication, local immutable-artifact preflight, explicit sampler-conformance decisions, and CPU benchmark metrics are testable. The pinned RFdiffusion v1 adapter has byte-repeatable passing A100 candidates on 8CZ8, independent 8B01 regular loops, and target-chain-only 7X35 glycine/proline-rich loops after deterministic fixed-atom post-sampling refinement, with measured MODELLER comparators; interface/insertion-code strata, complete residue/backbone-dependent rotamer validation, full-atom Protenix/Boltz adapters, and public dispatch remain open |
