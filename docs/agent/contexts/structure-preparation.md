...
| Diffusion protocol and RFdiffusion benchmark | partial | `model/diffusion/contract.py`, `masks.py`, `scope.py`, `geometry.py`, `boundary_refinement.py`, `runner.py`, `validate.py`, `pipeline.py`, `preflight.py`, `provenance.py`, `benchmark.py`, `sampler.py`, `rfdiffusion_v1.py` | Versioned protocol, containment, independent validation, provenance, publication, and benchmark metrics are implemented. The pinned RFdiffusion v1 adapter has byte-repeatable passing A100 candidates and MODELLER comparisons across 8CZ8, 8B01, difficult and interface-adjacent 7X35, and insertion-code 7K8S cases. A minimal digest-pinned container recipe keeps the checkpoint as a verified mount; final image smoke awaits an OCI runtime. Complete residue-dependent validation, all-atom Protenix/Boltz adapters, and public dispatch remain open. |
