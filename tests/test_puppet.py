...
def test_puppet_strips_sidechains(tmp_workdir: Path, hinge_ch3_glycosylated_pdb: Path) -> None:
...
def test_puppet_renames_to_gly(tmp_workdir: Path, hinge_ch3_glycosylated_pdb: Path) -> None:
    out = tmp_workdir / "puppet.pdb"
    main([str(hinge_ch3_glycosylated_pdb), "-o", str(out)])
    for ln in _iter_atoms(out):
...
def test_puppet_keep_preserves_original(tmp_workdir: Path, hinge_ch3_glycosylated_pdb: Path) -> None:
    """--keep should retain the residue's original name + all atoms."""
    # The hinge fixture starts with CYX at H:1.
    out = tmp_workdir / "puppet_keep.pdb"
    main([str(hinge_ch3_glycosylated_pdb), "-o", str(out), "--keep", "H:1"])
...
def test_puppet_prints_summary(tmp_workdir: Path, hinge_ch3_glycosylated_pdb: Path, capsys: pytest.CaptureFixture[str]) -> None:
    out = tmp_workdir / "puppet.pdb"
    main([str(hinge_ch3_glycosylated_pdb), "-o", str(out)])
