...
**`--acpype` mode:** Alternative to RTP-based topology. Uses `acpype_export.py` shared module: OpenMM (AMBER14+GLYCAM) → ParmEd → ACPYPE → GROMACS `topol.top`/`.gro` with `[ pairs_nb ]` for mixed 1-4 scaling. Output includes `#ifdef POSRES` / `#include "posre_{stem}.itp"` / `#endif` in the moleculetype section, and water/ion moleculetypes appended before `[ system ]`. Ignores `--ff`/`--water`/`--ignh`/`--merge` flags. Respects `--ss` for explicit disulfide bonds. Handles glycosylated proteins: auto-detects SS bonds from CONECT, reorders chains (protein first, glycan after), filters glycan TER records, captures/restores AMBER variant names across PDBFile normalization, adds peptide bonds for GLYCAM protein residues (NLN), adds HD21 for NLN, uses `ignoreExternalBonds=True` and `residueTemplates` for CYX disambiguation. **Terminal ASH/GLH limitation:** AMBER14 has no NASH/NGLH templates (never parameterized via RESP). Terminal ASH/GLH are converted to standard ASP/GLU with protonation H stripped; emits `UserWarning`.
