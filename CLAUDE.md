...
- Glycolipids (ceramide + sugar tree) built as single moleculetype by `build_glycolipid_chain()`. Ceramide from `lipid.rtp`, sugars from `carb.rtp`. Ceramide-sugar bond: C1S—O1 (sugar O1 bridges). Ceramide O1+HO1 charge → C1S. Sugar O1 type: OC301 (not OC3C61). CHARMM-GUI 4-char resnames (CER1, BGLC, etc.) detected via `read_pdb_chains()` extended column parsing.
